...
<form method="post" action="{{ route('preorder.postcreate2') }}" enctype="multipart/form-data">
    ...
    <table class="table" id="example1" width="100%" cellspacing="0">
      <tr>
        ...
        <td><select name="in_type">
          ...
          <option value="{{ $row->key ?? '' }}" {{ (request()->session()->get('sbdata.type') == $row->key) ? 'selected' : '' }}>{{$row->detail}}</option>
          @endforeach
        </select>
      </td>
    </tr>
    <tr>
      <td>Kain Inner</td><td><input type="text" class="form-control" name="in_finner" value="{{ $sbdata->finner ?? '' }}"></td>
    </tr>
    <tr>
      <td>Kain Outer</td><td><input type="text" class="form-control" name="in_fouter" value="{{ $sbdata->fouter ?? '' }}"></td>
    </tr>
    <tr>
      <td>Pola</td>
      <td><select name="in_pattern">
        ...
        <option value="{{ $row->key ?? '' }}" {{ (request()->session()->get('sbdata.pattern') == $row->key) ? 'selected' : '' }}>{{$row->detail}}</option>
        @endforeach
      </select>
    </td>
  </tr>
  <tr>
    ...
    <td><select name="in_fillw">
      ...
      <option value="{{ $row->key ?? '' }}" {{ (request()->session()->get('sbdata.fillw') == $row->key) ? 'selected' : '' }}>{{$row->detail}}</option>
      @endforeach
    </select>
  </td>
</tr>
<tr>
  ...
  <td><select name="in_size">
    ...
    <option value="{{ $row->key ?? '' }}" {{ (request()->session()->get('sbdata.size') == $row->key) ? 'selected' : '' }}>{{$row->detail}}</option>
    @endforeach
  </select>
</td>
</tr>
<tr>
  <td>Note</td><td><input type="text" class="form-control" name="in_note" value="{{ $preorder->note ?? '' }}"></td>
</tr>
</table>
</div>
<div>
